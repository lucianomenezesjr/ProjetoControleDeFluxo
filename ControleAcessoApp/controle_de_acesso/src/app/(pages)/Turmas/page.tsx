...
export default function UsuariosPage() {
  ...
  const fetchTurmas = async (token: string) => {
    try {
      const response = await fetch("http://127.0.0.1:7292/api/Turma", {
        headers: {
  ...
  const handleDeleteTurma = async (turmaId: number) => {
    ...
    try {
      const response = await fetch(`http://127.0.0.1:7292/api/Turma/${turmaId}`, {
        method: "DELETE",
        headers: {
          Authorization: `Bearer ${token}`,
        },
  ...
  const handleEditTurma = async (updatedTurma: Turma) => {
    ...
    try {
      const response = await fetch(`http://127.0.0.1:7292/api/Turma/${updatedTurma.id}`, {
        method: "PUT",
        headers: {
          Authorization: `Bearer ${token}`,
