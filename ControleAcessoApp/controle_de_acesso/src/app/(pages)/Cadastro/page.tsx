...
export default function Cadastro() {
  ...
  const [passwordConfirmation, setPasswordConfirmation] = useState(""); // Estado para confirmação de senha
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        toast.success("Cadastro realizado com sucesso! Redirecionando para login...", { duration: 1500 });
        setTimeout(() => {
          router.push("/Login");
        }, 1000);
      } else {
        toast.error(data.message || "Erro ao realizar cadastro", { duration: 3000 });
      }
    } catch (error) {
